...
public class Sugerencia {
	...
	public LinkedList<Oferta> armarColaOferta(LinkedList<Promocion> promociones,LinkedList<Atraccion> atracciones, double presupuesto, double tiempo){
		LinkedList<Oferta> cola = new LinkedList<Oferta>();
		for (Promocion pr : promociones) {
			if ((presupuesto >= pr.getCosto()) && (tiempo >= pr.getDuracion())
					&& (pr.getCupo()>0)) {
				Oferta ofer = new Oferta(pr.getNombre(), pr.getAtracciones(), pr.getCosto(), pr.getDuracion(), pr.getCupo());
				cola.add(ofer);
			}
		}
		for (Atraccion at : atracciones) {
			if ((presupuesto >= at.getCosto()) && (tiempo >= at.getDuracion())
					&& (at.getCupo()>0)) {
				Oferta ofer = new Oferta(at.getNombre(), at.getAtraccion(), at.getCosto(), at.getDuracion(), at.getCupo());
				cola.add(ofer);
			}
		}
		return cola;
